defmodule Waffle.Storage.Google.UrlV2 do
  ...
  def build(definition, version, meta, options) do
    ...
    if signed?(options) do
      ...
      build_url(definition, path, options)
    end
  end

  @spec build_url(Types.definition(), String.t(), Keyword.t()) :: String.t()
  defp build_url(definition, path, options \\ []) do
  ...
  defp build_signed_url(definition, path, options) do
    ...
    signature =
      definition
      |> build_path(path, include_bucket?)
  ...
  @spec build_path(Types.definition(), String.t(), boolean()) :: String.t()
  defp build_path(definition, path, include_bucket?) do
    case include_bucket? do
      true -> bucket_and_path(definition, path)
      false -> path
    end
    |> Util.prepend_slash()
    |> URI.encode()
